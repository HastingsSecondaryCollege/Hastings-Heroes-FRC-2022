...
PathPlannerTrajectory PathPlanner::loadPath(std::string name, units::meters_per_second_t maxVel, units::meters_per_second_squared_t maxAccel, bool reversed)
{
    ...
    double InternalMaxVel=json.at("maxVelocity");
    double InternalMaxAccel=json.at("maxAcceleration");

    //fmt::print("File Name: {}\n",name+".path");      
    fmt::print("....................\n.File Name: {}\n MaxVel from file: {}\n. MaxVel from Code: {}\n",
                name+".path",InternalMaxVel,maxVel.value());
    fmt::print(" MaxAccel from file : {}\n MaxAccel from code : {}\n......................\n",
                InternalMaxAccel,maxAccel.value());
    ...
    if (maxVel.value()==0) maxVel=units::meters_per_second_t(InternalMaxVel);
    if (maxAccel.value()==0) maxAccel=units::meters_per_second_squared_t(InternalMaxAccel);
    
    for (int i = 0; i < (int)splitPaths.size(); i++)
    {
        paths.push_back(PathPlannerTrajectory(splitPaths[i], maxVel, maxAccel, shouldReverse));
        // paths.push_back(PathPlannerTrajectory(splitPaths[i], units::meters_per_second_t(InternalMaxVel),units::meters_per_second_squared_t(maxAccel), shouldReverse));
